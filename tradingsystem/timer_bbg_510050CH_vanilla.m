...
etfvanillaTimer.StartFcn = @(~,thisEvent)disp([datestr(thisEvent.Data.time),' starts']);
...
etfvanillaTimer.StopFcn = @(~,thisEvent)disp([datestr(thisEvent.Data.time),' stops']);

%Specify the value of the TimerFcn callback. The example specifies the 
%MATLAB commands in a text string:;
etfvanillaTimer.TimerFcn = {@callback_bbg_510050CH_vanilla,c,strikes,4};
