...
if isempty(idx)
    ret = {};
    if debugflag
        fprintf('%s:invalid input openid as no open long signal was found then....\n',code);
    end
    return
end
...
if b1type == 1
    ret = {};
    if debugflag
        fprintf('%s:invalid input openid as weak long signal was found then...\n',code);
    end
    return
end

d = fractal_truncate(ei,openid);
op = fractal_filterb1_singleentry(b1type,nfractal,d,ticksizeratio*asset.tick_size); 
statusstruct = fractal_b1_status(nfractal,d,ticksizeratio*asset.tick_size);
...
for k = checkstartid:size(ei.px,1)
    ...
    elseif trade.oneminb4close1_ == 899 && isnan(trade.oneminb4close2_)
        if ~(strcmpi(freq,'intraday-30m') || strcmpi(freq,'intraday')), error('fractal_tradeinfo_anyb:invalid freq input....');end
