if ~(exist('conn','var') && isa(conn,'cBloomberg')), conn = cBloomberg;end
code_bbg_underlier = '510300 CH Equity';
%% 5y historical data
dt1 = dateadd(today,'-5y');
dt2 = today;
hd = conn.history(code_bbg_underlier,{'px_open','px_high','px_low','px_last','volume','open_int'},dt1,dt2);
rt = [hd(2:end,1),log(hd(2:end,5)./hd(1:end-1,5))];
[res] = bkfunc_hvcalib(rt,'forecastperiod',21, 'printresults',false, 'plotconditonalvariance',true,'scalefactor',sqrt(246));

%%
exp_feb = '2020-02-26';
exp_mar = '2020-03-25';
exp_jun = '2020-06-24';
k = 3.6:0.1:4.4;
opt_c_feb = cell(length(k),1);opt_p_feb = opt_c_feb;
opt_c_mar = cell(length(k),1);opt_p_mar = opt_c_mar;
opt_c_jun = cell(length(k),1);opt_p_jun = opt_c_jun;
for i = 1:length(k)
    opt_c_feb{i} = ['510300 CH ',datestr(exp_feb,'mm/dd/yy'),' C',num2str(k(i)),' Equity'];
    opt_p_feb{i} = ['510300 CH ',datestr(exp_feb,'mm/dd/yy'),' P',num2str(k(i)),' Equity'];
    opt_c_mar{i} = ['510300 CH ',datestr(exp_mar,'mm/dd/yy'),' C',num2str(k(i)),' Equity'];
    opt_p_mar{i} = ['510300 CH ',datestr(exp_mar,'mm/dd/yy'),' P',num2str(k(i)),' Equity'];
    opt_c_jun{i} = ['510300 CH ',datestr(exp_jun,'mm/dd/yy'),' C',num2str(k(i)),' Equity'];
    opt_p_jun{i} = ['510300 CH ',datestr(exp_jun,'mm/dd/yy'),' P',num2str(k(i)),' Equity'];
end