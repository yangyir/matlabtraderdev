...
for i = 1:length(opt300_c_aug)
    code_bbg = opt300_c_aug{i};
    try
        fn = [code_bbg(1:6),'_',datestr(code_bbg(11:18),'mmmyy'),'_',code_bbg(20:end-7),'_daily.txt'];
        data = cDataFileIO.loadDataFromTxtFile(fn);
        volume_opt300_c_aug(i,1) = data(end-1,6);
        volume_opt300_c_aug(i,2) = data(end,6);
    catch
    end
    %
    code_bbg = opt300_p_aug{i};
    try
        fn = [code_bbg(1:6),'_',datestr(code_bbg(11:18),'mmmyy'),'_',code_bbg(20:end-7),'_daily.txt'];
        data = cDataFileIO.loadDataFromTxtFile(fn);
        volume_opt300_p_aug(i,1) = data(end-1,6);
        volume_opt300_p_aug(i,2) = data(end,6);
    catch
    end
end
pcratio = sum(volume_opt300_p_aug(:,2))/sum(volume_opt300_c_aug(:,2));
fprintf('%15s\t%4.2f\n','pc-ratio',pcratio);
%
%
n_opt300_c_aug = length(opt300_c_aug);
bd_opt300_c_aug = cell(n_opt300_c_aug,1);
tbl_opt300_c_aug = [k_300',zeros(n_opt300_c_aug,3)];
cobdate = hd_300etf(end,1);
for i = 1:length(opt300_c_aug)
    try
        bd_opt300_c_aug{i} = pnlriskbreakdownbbg(opt300_c_aug{i},datenum(cobdate));
        tbl_opt300_c_aug(i,2) = bd_opt300_c_aug{i}.iv1;
        tbl_opt300_c_aug(i,3) = bd_opt300_c_aug{i}.iv2;
        tbl_opt300_c_aug(i,4) = bd_opt300_c_aug{i}.deltacarry/bd_opt300_c_aug{i}.spot2/10000;
    catch
        bd_opt300_c_aug{i} = [];
        tbl_opt300_c_aug(i,2:4) = NaN;
    end
end
% PUT
n_opt300_p_aug = n_opt300_c_aug;
bd_opt300_p_aug = cell(n_opt300_p_aug,1);
tbl_opt300_p_aug = [k_300',zeros(n_opt300_p_aug,3)];
for i = 1:n_opt300_p_aug
    try
        bd_opt300_p_aug{i} = pnlriskbreakdownbbg(opt300_p_aug{i},datenum(cobdate));
        tbl_opt300_p_aug(i,2) = bd_opt300_p_aug{i}.iv1;
        tbl_opt300_p_aug(i,3) = bd_opt300_p_aug{i}.iv2;
        tbl_opt300_p_aug(i,4) = bd_opt300_p_aug{i}.deltacarry/bd_opt300_p_aug{i}.spot2/10000;
    catch
        bd_opt300_p_aug{i} = [];
        tbl_opt300_p_aug(i,2:4) = NaN;
    end
end
m1 = zeros(length(opt300_c_aug),1);
m2 = zeros(length(opt300_c_aug),1);
for i = 1:length(opt300_c_aug)
    try
        m1(i) = tbl_opt300_c_aug(i,1)/bd_opt300_c_aug{i}.fwd1;
    catch
        m1(i) = NaN;
    end
    try
        m2(i) = tbl_opt300_c_aug(i,1)/bd_opt300_c_aug{i}.fwd2;
    catch
        m2(i) = NaN;
    end
end
...
m1 = m1(idx);
m2 = m2(idx);
tbl_opt300_c_aug = tbl_opt300_c_aug(idx,:);
m = max(min(m1),min(m2)):0.01:min(max(m1),max(m2));
volinterp1 = interp1(m1,tbl_opt300_c_aug(:,2),m);
volinterp2 = interp1(m1,tbl_opt300_c_aug(:,3),m);
atmf1 = interp1(m1,tbl_opt300_c_aug(:,2),1);
atmf2 = interp1(m1,tbl_opt300_c_aug(:,3),1);
fprintf('%15s\t%4.1f%%\n','atmfvol:',atmf2*100);
