classdef cStratFutPairCointegration < cStrat
    ...
    methods
        function [ret] = initdata(obj,fromdate,todate,interval)
            %note:for now we only do with intraday bar data
            %todo:maybe end-of-day data will be implmented as well
            ...
            try
                ...
                d1 = ds.intradaybar(instrument1,fromdate,todate,interval,'trade');
                d2 = ds.intradaybar(instrument2,fromdate,todate,interval,'trade');
            
                [t,idx1,idx2] = intersect(d1(:,1),d2(:,1));
                obj.data_ = [t,d1(idx1,end),d2(idx2,end)];
                
                count = size(obj.data_,1);
                M = obj.lookbackperiod_;
                N = obj.rebalanceperiod_;
                idx = min(max(M,N),count);
