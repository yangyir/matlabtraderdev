function [] = processcondentrust( stratoptfractal, varargin )
% a cStratOptMultiFractal
%process any EXISTING conditional entrust associated with input instrument
ncondpending = stratoptfractal.helper_.condentrustspending_.latest;
...
%instruments are list of options (ONLY) 
...
for jj = 1:ncondpending
    condentrust = stratoptfractal.helper_.condentrustspending_.node(jj);
    %NOW check with underlier code only
    %GOING FORWARD checks with options on such underlier shall be included
    if ~strcmpi(underlier.code_ctp,condentrust.instrumentCode), continue;end
    ...
    if ~isempty(strfind(signalinfo.mode,'conditional-uptrendconfirmed')) || strcmpi(signalinfo.mode,'conditional-uptrendbreak')
        ...
        if ispxbelowteeth || islatesthhlower || ishhupdated
            ...
            if ispxbelowteeth
                fprintf('\t%6s:%s:%s\n',underlier.code_ctp,signalinfo.mode, 'canceled as price falls below alligator teeth...');
            elseif islatesthhlower
                fprintf('\t%6s:%s:%s\n',underlier.code_ctp,signalinfo.mode, 'canceled as new hh is lower...');
            elseif ishhupdated
                fprintf('\t%6s:%s:%s\n',underlier.code_ctp,signalinfo.mode, 'canceled as new hh is higher...');
            end
        end
        %
    elseif ~isempty(strfind(signalinfo.mode,'conditional-dntrendconfirmed')) || strcmpi(signalinfo.mode,'conditional-dntrendbreak')
        ...
        if ispxaboveteeth || islatestllhigher || isllupdated
            condentrusts2remove.push(condentrust);
            if ispxaboveteeth
                fprintf('\t%6s:%s:%s\n',underlier.code_ctp,signalinfo.mode, 'canceled as price rallies above alligator teeth...');
            elseif islatestllhigher
                fprintf('\t%6s:%s:%s\n',underlier.code_ctp,signalinfo.mode, 'canceled as new ll is higher...');
            elseif isllupdated
                fprintf('\t%6s:%s:%s\n',underlier.code_ctp,signalinfo.mode, 'canceled as new ll is lower...');
            end
        end
        %    
    elseif strcmpi(signalinfo.mode,'conditional-close2lvlup')
        ...
        if p(end,5) < hh(end)
            condentrusts2remove.push(condentrust);
            fprintf('\t%6s:%s:%s\n',underlier.code_ctp,signalinfo.mode, 'canceled as close price falls below hh...');
        end
        %
    elseif strcmpi(signalinfo.mode,'conditional-close2lvldn')
        ...
        if p(end,5) > ll(end)
            condentrusts2remove.push(condentrust);
            fprintf('\t%6s:%s:%s\n',underlier.code_ctp,signalinfo.mode, 'canceled as close price rallied above ll...');
        end
        %
        ...
        if p(end,3) < lvldn(end) || hh(end-1) ~= hh(end)
            ...
            if isteethjawcrossed && ss(end) >= 8
                ...
                if wad(maxpxidx) >= wad(end)
                    ...
                    condentrusts2remove.push(condentrust);
                    fprintf('\t%6s:%s:%s\n',underlier.code_ctp,signalinfo.mode, 'canceled as teeth jaw crossed..');
                end 
            end
        end
        %
    elseif strcmpi(signalinfo.mode,'conditional-breachdnlvldn')
        ...
        if p(end,4) > lvlup(end) || ll(end-1) ~= ll(end)
            ...
            if p(end,4) > lvlup(end)
                fprintf('\t%6s:%s:%s\n',underlier.code_ctp,signalinfo.mode, 'canceled as lowest price rallied above lvlup...');
            elseif ll(end-1) ~= ll(end)
                fprintf('\t%6s:%s:%s\n',underlier.code_ctp,signalinfo.mode, 'canceled as ll value updated...');
            end
        else
