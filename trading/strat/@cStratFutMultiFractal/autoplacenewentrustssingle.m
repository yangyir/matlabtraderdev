function [] = autoplacenewentrustssingle(stratfractal,varargin)
    ...
    iparser.addParameter('Signals',[],@iscell);
    iparser.parse(varargin{:});    
    signals = iparser.Results.Signals;
    ...
    if isempty(stratfractal.helper_), error('%s::autoplacenewentrustssingle::missing helper!!!',class(stratfractal));end

    signal_long = signals{1};
    signal_short = signals{2};
    ...
    if volume == 0 || maxvolume == 0, return;end
            ...
            if bid < signal_short(3) && bid > signal_short(3)-1.618*(signal_short(2)-signal_short(3))
                ...
                if isvolblowup2 && ~isbslowbreach
                    ...
                    if ~strcmpi(freq,'1440m')
                        try
                            kelly = kelly_k('volblowup2',instrument.asset_name,stratfractal.tbl_all_intraday_.signal_s,stratfractal.tbl_all_intraday_.asset_list,stratfractal.tbl_all_intraday_.kelly_matrix_s);
                        catch
                        ...
                        try
                            kelly = kelly_k('volblowup2',instrument.asset_name,stratfractal.tbl_all_daily_.signal_s,stratfractal.tbl_all_daily_.asset_list,stratfractal.tbl_all_daily_.kelly_matrix_s);
                        catch
        ...
        if signal_long(4) == 2 || signal_long(4) == 3 || signal_long(4) == 4
            ...
            if ask > signal_long(2)+ticksize && ask < signal_long(2)+1.618*(signal_long(2)-signal_long(3))
                ...
                if isvolblowup2 && ~issshighbreach
                    ...
                    if ~strcmpi(freq,'1440m')
                        try
                            kelly = kelly_k('volblowup2',instrument.asset_name,stratfractal.tbl_all_intraday_.signal_l,stratfractal.tbl_all_intraday_.asset_list,stratfractal.tbl_all_intraday_.kelly_matrix_l);
                        catch
                        ...
                        try
                            kelly = kelly_k('volblowup2',instrument.asset_name,stratfractal.tbl_all_daily_.signal_l,stratfractal.tbl_all_daily_.asset_list,stratfractal.tbl_all_daily_.kelly_matrix_l);
                        catch
