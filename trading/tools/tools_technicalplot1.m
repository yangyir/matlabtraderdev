...
wad = williamsad(p);
atr = calcATR(p(:,3),p(:,4),p(:,5));
ao = calcAO(p(:,3),p(:,4));
[topf,botf] = identifyfractals(p(:,3),p(:,4),nfractal);
outputmat = [m2xdate(p(:,1)),p(:,2:5),idxHH,idxLL,HH,LL,jaw,teeth,lips,bs,ss,lvlup,lvldn,bc,sc,wad,atr,ao,topf,botf];
