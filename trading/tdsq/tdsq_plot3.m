function [output] = tdsq_plot3(p,idxstart,idxend,instr2,cpnl,signals)
...
cpnl2plot = cpnl(idxstart:idxend);
signals2plot = signals(idxstart:idxend);
...
ax(3) = subplot(4,1,3);
plot(cpnl2plot,'b');grid on;
set(ax(3),'XTickLabel',xticklabel,'fontsize',8);grid on;
ylabel('pnl');
xlabel('serial time number')

ax(4) = subplot(4,1,4);
stairs(signals2plot);
set(ax(4),'XTickLabel',xticklabel,'fontsize',8);grid on;
ylabel('position');
xlabel('serial time number')
linkaxes(ax,'x')
