function [output] = tdsq_plot2withboundary(p,idxstart,idxend,instr2)
...
for i = 2:np
    [ub(i),lb(i)] = tdsq_updateboundary(p(1:i,1:5),tdBuySetup(1:i),tdSellSetup(1:i),tdSTResistence(1:i),tdSTSupport(1:i),ub(i-1),lb(i-1));
end
...
ax(1) = subplot(2,1,1);
stairs(ub(idxstart:idxend),'r:','LineWidth',2);hold on;
stairs(lb(idxstart:idxend),'g:','LineWidth',2);
if isa(instr2,'cInstrument')
