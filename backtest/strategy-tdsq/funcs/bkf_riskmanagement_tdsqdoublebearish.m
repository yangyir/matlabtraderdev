function [ ret,closestr,tradeout,closeidx ] = bkf_riskmanagement_tdsqdoublebearish( tradein,p,bs,ss,bc,sc,macdvec,sigvec,macdbs,macdss,sns,varargin )
    %reverse mode 
    variablenotused(bc);
    variablenotused(sc);
    try
        type = tradein.opensignal_.type_;
        if ~strcmpi(type,'double-bearish') 
            ret = false;
            closestr = '';
            tradeout = {};
            closeidx = -1;
...
%     closeoncountdown13 = iparser.Results.closeoncountdown13;
    ...
    elseif tradein.opendirection_ == 1
        ...
        for j = openidx+1:n
            ...
            %
            if hasbreachlvldn && p(j,3) < lvldn_open
                closestr = 'breachbounceback';
                break;
            end
            %
            if ~hasbreachlvldn && p(j,3) < lvlup_open
                closestr = 'trendreverse';
                break
            end
            %
