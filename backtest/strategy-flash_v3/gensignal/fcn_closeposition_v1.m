...
function [opensignal_outside_highest,opensignal_outside_lowest,opensignal_highest,opensignal_lowest] = fcn_closeposition_v1(candles)
        ...
        for i = 1:N_opensignal_highest
            ...
            if ~isempty(v) 
                ...
                if isnan(opensignal_highest{i}.pnl)
                    stoplosslocation = find(candles(startlocation:end,3)>= opensignal_highest{i}.stoploss1);
                else
                    lastlocation = find (candles(:,1)==opensignal_highest{i}.closetimenum);
                    stoplosslocation = find(candles(startlocation:lastlocation,3)>= opensignal_highest{i}.stoploss1);
                end
                if ~isempty(stoplosslocation)
                    ...
                    opensignal_highest{i}.closeprice = opensignal_highest{i}.stoploss1;
                    opensignal_highest{i}.pnl = (opensignal_highest{i}.closeprice - opensignal_highest{i}.openprice)*opensignal_highest{i}.direction*opensignal_highest{i}.N_position;
                end
                v=[];vv=[];vv1=[];vv2=[];
        end
        ...
        for i = 1:N_opensignal_lowest
            ...
            if ~isempty(v)     
                ...
                if isnan(opensignal_lowest{i}.pnl)
                    stoplosslocation = find(candles(startlocation:end,4)<= opensignal_lowest{i}.stoploss1);
                else
                    lastlocation = find (candles(:,1)==opensignal_lowest{i}.closetimenum);
                    stoplosslocation = find(candles(startlocation:lastlocation,4)<= opensignal_lowest{i}.stoploss1);
                end
                if ~isempty(stoplosslocation)
                    ...
                    opensignal_lowest{i}.closeprice = opensignal_lowest{i}.stoploss1;
                    opensignal_lowest{i}.pnl = (opensignal_lowest{i}.closeprice - opensignal_lowest{i}.openprice)*opensignal_lowest{i}.direction*opensignal_lowest{i}.N_position;
                end
                v=[];vv=[];vv1=[];vv2=[];
        end
